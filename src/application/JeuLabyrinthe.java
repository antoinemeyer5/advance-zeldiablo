...
public class JeuLabyrinthe implements Jeu {
	...
	public boolean etreFini() {
		...
		if(this.getLabyrinthe().getAventurier().getHasAmulette() && this.getLabyrinthe().getAventurier().getX() == this.lab.getXSortie() && this.getLabyrinthe().getAventurier().getY() == this.lab.getYSortie()) {
			...
			int s = lab.getAventurier().getEtageParcouru();
			//on enregistre les pdv
			int p = lab.getAventurier().getVie();
			//on crée un nouveau labyrinthe
			this.lab = new Labyrinthe(11);
			//on remet le score
			this.getLabyrinthe().getAventurier().setEtageParcouru(s+1);
			//on remet les pdv
			this.getLabyrinthe().getAventurier().setVie(p);
		}
		//renvoie du resultat
		return end;
	}
