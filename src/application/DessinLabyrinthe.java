...
public class DessinLabyrinthe implements DessinJeu {
	...
	public void dessiner(BufferedImage image) {
		...
		if(!jeu.getLabyrinthe().getAventurier().getHasAmulette()){
			g.drawImage(sprites.get(8),jeu.getLabyrinthe().getObjets().get(0).getX() * TAILLE,jeu.getLabyrinthe().getObjets().get(0).getY() * TAILLE,null);
		}
		//affichage des potions de vie
		if(jeu.getLabyrinthe().getObjets().size()==2){
			if(jeu.getLabyrinthe().getAventurier().hasPotion==false){
				g.drawImage(sprites.get(17),jeu.getLabyrinthe().getObjets().get(1).getX() * TAILLE,jeu.getLabyrinthe().getObjets().get(1).getY() * TAILLE,null);
			}
		}
	...
	public void chargerSprites() {
		try {
			...
			//potion - 17
			sprites.add(ImageIO.read(new File("src/ressources/potion.png")));
